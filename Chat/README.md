...
上传后的头像文件会保存在后端项目目录下的 `uploads/avatars`，浏览器通过 `/uploads/avatars/文件名` 访问。

## 7. 调试两个用户聊天

浏览器的登录态保存在 Cookie 里。同一个浏览器的普通标签页会共用同一份登录态，所以不能用两个普通标签页分别登录两个账号测试聊天。

正确方式：

- 用两个不同浏览器，例如 Edge 登录账号 A，Chrome 登录账号 B
- 或者一个普通窗口登录账号 A，一个无痕窗口登录账号 B
- 或者使用两个独立浏览器用户配置文件

如果在同一个浏览器里先登录 A，再登录 B，A 标签页也会变成 B 的登录态。这时在 A 原来的聊天窗口里发给 B，后端会判断成“B 给 B 发消息”，于是返回“不能和自己聊天”。
